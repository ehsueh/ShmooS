...
delta_mt = (mbg*nex_gef + nex_intr)*md - hyd_42*mt + att_42bc*mbg*cd + D_2*laplacian(mt, [R_, thet, phi], Spherical[RightHanded]);

function delta_md = delta_md(mbg, md, mt, cd, thet, phi)
delta_md = -(mbg*nex_gef + nex_intr)*md + hyd_42*mt + att_42*cd - ext_42*md + D_2*laplacian(md, [R_, thet, phi], Spherical[RightHanded]);

function delta_mb = delta_mb(mt, mb, mb, cg, thet, phi)
delta_mb = att_b*mt*cb - det_b*mb*cg - att_24*mb*cb + det_24*mbg + D_2*laplacian(mb, [R_, thet, phi], Spherical[RightHanded] );

function delta_bg = delta_bg(mb, cg, mbg, thet, phi)
delta_bg = att_24*mb*cb - det_24 - det_24*mbg + D_2*laplacian(mbg, [R_, thet, phi], Spherical[RightHanded]);

function delta_cg = delta_cg(cd, r, thet, phi) 
delta_cg = D_3 * laplacian(cd, [r, thet, phi], Spherical[RightHanded]);

function delta_cb = delta_cb(cb, r, thet, phi) 
delta_cb = D_3 * laplacian(cb, [r, thet, phi], Spherical[RightHanded]);

function delta_cg = delta_cg(cg, r, thet, phi) 
delta_cg = D_3 * laplacian(cg, [r, thet, phi], Spherical[RightHanded]);
